...
function Paragraph({ paragraph }: { paragraph: LegalParagraph }) {
  if (typeof paragraph === "string") {
    return <p className="mt-4 text-body text-ink-secondary">{paragraph}</p>;
  }
      ...
      <Container>
        ...
        <div className="mt-12 grid gap-10 lg:mt-16 lg:grid-cols-12 lg:gap-12">
          <div className="lg:col-span-7 xl:col-span-8">
            <p
              role="note"
              className="rounded-card border border-accent/40 bg-accent-tint px-5 py-4 text-body text-ink-secondary"
            >
          ...
          <aside className="lg:col-span-5 xl:col-span-4">
            <div className="lg:sticky lg:top-8">
              <nav aria-label="On this page">
                <p className="text-eyebrow text-ink-muted">On this page</p>
                <ol className="mt-5 divide-y divide-hairline border-y border-hairline">
                  {sections.map((section, index) => (
                    <li key={section.heading}>
                      <a
                        href={`#${slug(section.heading)}`}
                        className="flex items-baseline gap-4 py-3 text-body text-ink-secondary transition-colors hover:text-accent"
                      >
                        <span
                          aria-hidden="true"
                          className="tabular text-caption text-ink-muted"
                        >
              ...
              <div className="mt-10 rounded-card border border-hairline bg-surface p-6">
                <p className="text-heading text-ink">Something unclear?</p>
